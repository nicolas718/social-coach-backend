...
const ensureUserRecordWithUserId = async (userId, deviceId, actionDate) => {
  try {
    ...
    // First, check if user record exists with user_id
    ...
    // Check if there's an existing record with this device_id
    ...
    if (deviceError) {
      console.error('❌ [SUPABASE] Error checking device records:', deviceError);
      throw deviceError;
    }
    
    if (deviceUsers && deviceUsers.length > 0) {
      // Find a record without user_id to update
      const deviceUserToUpdate = deviceUsers.find(user => !user.user_id);
      
      if (deviceUserToUpdate) {
        console.log(`🔧 [SUPABASE] Found existing device record without user_id, updating with user_id: ${userId}`);
        ...
        console.log(`🔧 [SUPABASE] Device record exists but already has user_id, using existing record`);
        return deviceUsers[0]; // Return the first record
      }
    } else {
      console.log(`🔧 [SUPABASE] No device record found, creating new user record...`);
      
      // Create new user record with user_id - use UPSERT to handle conflicts
...
const updateUserStreakSupabaseByUserId = async (userId, actionDate) => {
  console.log(`=== SUPABASE STREAK DEBUG for user_id ${userId} ===`);
  ...
  try {
    ...
    if (user.last_completion_date) {
      ...
      console.log('[SUPABASE] Date comparison (by user_id):', {
        lastDateStr,
        currentDateStr,
    ...
    console.log(`[SUPABASE] Final streak values (by user_id): current=${newStreak}, best=${newBestStreak}`);
    
    // Update user record in Supabase (equivalent to SQLite UPDATE)
    ...
    if (updateError) {
      console.error('❌ [SUPABASE] Error updating user streak by user_id:', updateError);
      throw updateError;
    }
    
    console.log(`✅ [SUPABASE] User streak updated successfully by user_id: ${userId}`, updateData);
    ...
    console.error(`❌ [SUPABASE] Error in updateUserStreakSupabaseByUserId for ${userId}:`, error);
    throw error;
  }
};
...
// Duplicate auth endpoints removed - using the ones defined before middleware
